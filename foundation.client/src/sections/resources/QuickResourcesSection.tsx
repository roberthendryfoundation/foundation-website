            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    {quickResources.map((resource, index) => (
                        <Card key={index} className="border-border shadow-soft hover:shadow-lg transition-shadow">
                            <CardHeader className="text-center">
                                <div
                                    className={`rounded-full p-3 w-fit mx-auto mb-4 ${resource.type === 'emergency' ? 'bg-destructive/10' : 'bg-primary/10'
                                        }`}
                                >
                                    <resource.icon
                                        className={`h-6 w-6 ${resource.type === 'emergency' ? 'text-destructive' : 'text-primary'
                                            }`}
                                    />
                                </div>
                            ...
                            <CardContent className="text-center">
                                ...
                                <Button
                                    size="sm"
                                    className={resource.type === 'emergency' ? 'bg-destructive hover:bg-destructive/90' : ''}
                                >
                                    {resource.action}
                                </Button>
                            </CardContent>
